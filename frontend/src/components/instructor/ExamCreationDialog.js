...
const InstructorExamCreationDialog = ({ open, onClose, onSuccess, editMode = false, examData = null }) => {
  ...
  const fetchAvailableSubjects = async () => {
    try {
      ...
      setAvailableSubjects(response.data?.subjects || response.data?.data?.subjects || []);
    } catch (error) {
  ...
  const getTotalWeightage = () => formData.subjects.reduce((sum, s) => sum + s.weightage, 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editMode && examData?._id) {
        await instructorService.updateExam(examData._id, payload);
        toast.success('Exam updated successfully');
      } else {
        await instructorService.createExam(payload);
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth PaperProps={{ sx: { minHeight: '80vh' } }}>
      <DialogTitle>
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Chip label="Smart Question Selection" color="primary" size="small" icon={<InfoIcon />} />
        </Box>
      </DialogTitle>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            ...
            <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
              ...
              <Button variant="outlined" startIcon={<AddIcon />} onClick={addSubject} size="small">
                Add Subject
              </Button>
            </Box>
            {formData.subjects.length === 0 ? (
              <Alert severity="info">Add subjects and their weightages. The total must equal 100%.</Alert>
            ) : (
              <Box>
