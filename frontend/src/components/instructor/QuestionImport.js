...
const InstructorQuestionImport = ({ open, onClose, onSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    // Use the same field name as admin CSV import
    ...
    try {
      const response = await api.post('/instructor/questions/import-csv', formData, {
        headers: {
          ...
          <Box sx={{ mb: 3 }}>
            <Card sx={{ mb: 3 }}>
              <CardContent>
                ...
                <Button variant="text" onClick={() => setShowInstructions(false)}>
                  Skip Instructions
                </Button>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent>
                ...
                <Alert severity="info" sx={{ mt: 2 }}>
                  <strong>Tips:</strong>
                  <br />• Use Excel (.xlsx) or CSV with the exact format: Serial, Question, Option A, Option B, Option C, Correct Option
                  <br />• Each question needs 2-3 options and specify the correct option (A, B, or C)
                  <br />• You can group questions by difficulty in the template (Easy/Medium/Hard sections)
                  <br />• File name should be the subject name (e.g., "Electronics.xlsx")
                </Alert>
              </CardContent>
            </Card>
          </Box>
            ...
            <Card sx={{ mb: 3 }}>
              <CardContent>
                ...
                <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
                  <Chip icon={<Info />} label={`${importResult.summary.totalRows} Total Rows`} />
                  <Chip
                    icon={<CheckCircle />}
